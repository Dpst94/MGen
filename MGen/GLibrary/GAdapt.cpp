...
void CGAdapt::AdaptAheadStep(int v, int x, int i, int ii, int ei, int pi, int pei) {
	...
	if (i > 0 && pi < i) {
		if (icf[ii].legato_ahead[0] > 0 && (artic[i][v] == aSLUR || artic[i][v] == aLEGATO) &&
			...
			float pdur = (setime[i - 1][v] - sstime[pi][v]) * 100 / m_pspeed +
			...
			// Decrease max_shift if it is greater than previous note length
			if (icf[ii].legato_ahead[0] + max_shift / 2 >	pdur) {
				max_shift = (pdur - icf[ii].legato_ahead[0] - 1) * 2;
			}
			// Calculate real randomized ahead
...
void CGAdapt::Adapt(int step1, int step2) {
	...
	for (int i = step1; i <= step2; i++) {
		...
		if (i > 0) {
			...
			// Create random
			tr = sr.inter_sig / sr.s_range * (float)rnd_tempo * (float)tempo_src[i] / 200.0 * fadeout;
