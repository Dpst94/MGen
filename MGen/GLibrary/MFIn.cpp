...
void MFIn::LoadCantus(CString path)
{
	long long time_start = CGLib::time();
	if (!fileExists(path)) {
		CString est;
...
void MFIn::ProcessInter(int pos, int pos_old, std::vector<std::vector<std::pair<int, int>>> &inter, int hid, std::vector<int> &min_len, std::vector<int> &max_len)
{
