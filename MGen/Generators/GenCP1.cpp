...
int CGenCP1::FailUnison() {
	...
	if (!civl[s]) {
		...
		if (s > 0 && bmli[s] == bmli[s - 1]) {
			if (civl[s - 1] == 1) FLAG2L(275, s, ssus[ls - 1]);
			else if (civl[s - 1] == 2) FLAG2L(277, s, ssus[ls - 1]);
		}
...
int CGenCP1::FailPcoSus() {
	...
	if (tivl[s] == iPco) {
		...
		if (civlc[s] == civlc[s - 1]) {
			if (civlc[s] == 7) FLAG2L(85, s - 1, s);
			else FLAG2L(482, s - 1, s);
		}
		// Prohibit different
	...
	if (sus[ls] && tivl[sus[ls]] == iPco && !retrigger[sus[ls]]) {
		...
		if (ep2 >= c_len || ls < fli_size - 1) {
			...
			if (civl[sus[ls]] == civl[fli2[ls - 1]]) {
				if (civlc[sus[ls]] == 7) {
					if (ivlc[fli[ls]] == 5)
						FLAG2L(330, sus[ls], ssus[ls - 1]);
					else
						FLAG2L(385, sus[ls], ssus[ls - 1]);
				}
				else FLAG2L(491, sus[ls], ssus[ls - 1]);
			}
		}
	}
	if (tivl[s] == iPco) {
		...
		if (civl[s] == civl[fli2[ls - 1]]) {
			if (civlc[s] == 7) FLAG2L(84, ssus[ls - 1], s);
			else FLAG2L(481, ssus[ls - 1], s);
		}
		else {
			// Prohibit contrary movement
			if (bmli[s] - 1 == bmli[fli2[ls - 1]] && civlc[s] == civlc[fli2[ls - 1]]) {
				if (civlc[s] == 7) FLAG2L(85, ssus[ls - 1], s);
				else FLAG2L(482, ssus[ls - 1], s);
			}
			// Prohibit different
			else if (tivl[fli2[ls - 1]] == iPco) {
				FLAG2L(86, ssus[ls - 1], s);
			}
			...
			if (motion[fli2[ls - 1]] == mDirect) {
				...
				if (abs(acc[1][s] - acc[1][s - 1]) < 3) {
					if (bmli[s] >= mli.size() - 2 && s > 0 && bmli[s - 1] != bmli[s]) {
						if (civlc[s] == 0) {
							if (civl[s] == 0) FLAG2L(72, ssus[ls - 1], s);
							else FLAG2L(209, ssus[ls - 1], s);
						}
						else FLAG2L(208, ssus[ls - 1], s);
					}
					else if (ls < fli_size - 1 || ep2 == c_len) {
						if (civlc[s] == 0) {
							if (civl[s] == 0) FLAG2L(76, ssus[ls - 1], s);
							else FLAG2L(211, ssus[ls - 1], s);
						}
						else FLAG2L(210, ssus[ls - 1], s);
					}
				}
				// Non-stepwize
				else {
					if (bmli[s] >= mli.size() - 2 && s > 0 && bmli[s - 1] != bmli[s]) {
						if (civlc[s] == 0) {
							if (civl[s] == 0) FLAG2L(73, ssus[ls - 1], s);
							else FLAG2L(213, ssus[ls - 1], s);
						}
						else FLAG2L(212, ssus[ls - 1], s);
					}
					else if (ls < fli_size - 1 || ep2 == c_len) {
						if (civlc[s] == 0) {
							if (civl[s] == 0) FLAG2L(98, ssus[ls - 1], s);
							else FLAG2L(215, ssus[ls - 1], s);
						}
						else FLAG2L(214, ssus[ls - 1], s);
					}
				}
			}
		}
...
void CGenCP1::ApplyFixedPat() {
	...
	for (int ls = 0; ls < fli_size; ++ls) {
		...
		if (pat[ls] == pPDD) {
			...
			if (tivl[fli[ls + 2]] == iDis || tivl[fli[ls + 1]] == iDis)
				ApplyCam2(ls, mshf, 1);
		}
	}
	// Set mshf for empty
		...
		for (int lp = 0; lp < l_len.size(); ++lp) {
			...
			if (l_len[lp] == 1) {
				...
				if (ls2 < fli_size - 1 && aleap[cpv][s2]) 
					...
					if (llen[ls2 - 1] > 1) FLAG2(88, ssus[bli[s2 - 1]]);
				}
			}
			else {
				// 1/8 syncope
			...
			if (skip_len > 0 && skip_len < (pco_apart * npm) / 4) {
				...
				if (retrigger[s]) {
					...
					if (ep2 == c_len && ls == fli_size - 1 &&
						...
						if (iv == 7) FLAG2L(376, s, pco_last);
						else FLAG2L(485, s, pco_last);
					}
					// Other anticipation
					else {
						if (iv == 7) FLAG2L(315, s, pco_last);
						else FLAG2L(489, s, pco_last);
					}
				}
				// Suspension
				else if (sus[ls] == s) {
				...
				// Downbeat
					...
					// Other downbeat
						...
						if (iv == 7) FLAG2L(316, s, pco_last);
						else FLAG2L(490, s, pco_last);
					}
				}
				...
				// Compound
					...
					if ((acc[0][s] - acc[0][pco_last]) * (acc[1][s] - acc[1][pco_last]) > 0) {
						if (iv == 7) FLAG2L(347, s, pco_last);
						else FLAG2L(486, s, pco_last);
					}
					// Contrary compound
					else {
						if (iv == 7) FLAG2L(248, s, pco_last);
						else FLAG2L(484, s, pco_last);
					}
				}
				// One of intervals is approached and left by stepwize movement 
					...
					(!sus[ls] && ls && asmooth[cpv][s - 1] &&
					(s2 + 1 >= ep2 || asmooth[cpv][s2]))) {
					if (iv == 7) FLAG2L(249, s, pco_last);
					else FLAG2L(488, s, pco_last);
				}
				// Asymmetric
				else if (beat[ls_1] != beat[ls]) {
					if (iv == 7) FLAG2L(374, s, pco_last);
					else FLAG2L(487, s, pco_last);
				}
				// Other
				else {
					if (iv == 7) {
		...
		if (tivl[s] == iIco && ivl[s] == ivl[fli2[ls - 1]]) {
			...
			if (pico_count == ico_chain-1) {
				if (civlc[s] == 3 || civlc[s] == 4) {
					FLAG2L(89, s, ssus[ls - 2]);
				}
				else {
					FLAG2L(389, s, ssus[ls - 2]);
				}
			}
			else if (pico_count >= ico_chain2) {
				if (!fired) {
					if (civlc[s] == 3 || civlc[s] == 4) {
						FLAG2L(96, s, ssus[ls - 3]);
					}
					else {
						FLAG2L(390, s, ssus[ls - 3]);
					}
					fired = 1;
				}
				else {
...
// Load counterpoint
void CGenCP1::LoadCP(CString path)
{
	long long time_start = CGLib::time();
	if (!fileExists(path)) {
		CString est;
...
void CGenCP1::ProcessInter(int pos, int pos_old, std::vector<std::vector<std::pair<int, int>>> &inter, int hid, std::vector<int> &min_len, std::vector<int> &max_len)
{
