...
void CP2D::LoadConfigLine(CString* sN, CString* sV, int idata, float fdata) {
	...
	CheckVar(sN, sV, "ly_max_partname_len", &ly_max_partname_len, 1, 100);
...
void CP2D::LoadRules(CString fname) {
	...
	while (fs.good()) {
		...
		if (st.Find(";") != -1) {
			...
			ruleinfo[rid].viz_notecolor = atoi(ast[14]);
			ruleinfo[rid].viz_harm = atoi(ast[15]);
			ruleinfo[rid].viz_sep = atoi(ast[16]);
			ruleinfo[rid].viz_text = ast[17];
			ruleinfo[rid].false_positives_global = atoi(ast[20]);
			ruleinfo[rid].false_positives_ignore = atoi(ast[21]);
			ruleinfo[rid].sas_emulator_max_delay = atoi(ast[22]);
			ruleinfo[rid].sas_emulator_move_ignore = atoi(ast[23]);
			if (!ast[24].IsEmpty()) {
			...
			ruleinfo[rid].sas_emulator_unstable = atoi(ast[25]);
			if (!ast[26].IsEmpty()) {
				Tokenize(ast[26], ast2, ",");
				for (int x = 0; x < ast2.size(); ++x) {
					fl = atoi(ast2[x]);
					if (fl >= ruleinfo.size()) ruleinfo.resize(fl + 1);
